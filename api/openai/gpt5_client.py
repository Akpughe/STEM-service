"""GPT-5 Pro client with vision capabilities."""
...
class GPT5Client:
    """Client for GPT-5 Pro with vision capabilities."""
    ...
    def __init__(self):
        ...
        self.model = model_mapping.get(settings.openai_model, settings.openai_model)
        self.max_completion_tokens = settings.openai_max_completion_tokens
    ...
    async def complete(
        ...
        try:
            ...
            if self.model in ["gpt-4o", "gpt-4-turbo", "gpt-4-turbo-preview"]:
                completion_params["max_completion_tokens"] = max_completion_tokens or self.max_completion_tokens
            else:
                completion_params["max_tokens"] = max_completion_tokens or self.max_completion_tokens
            ...
            logger.error("GPT-5 completion failed", error=str(e))
            raise
    ...
    async def parse_image_to_math(
        ...
        # Build messages
        system_prompt = """You are an expert at reading and interpreting mathematical notation, 
        equations, and scientific diagrams from images. Extract all mathematical content accurately,
        preserving the exact notation and structure."""
        
        user_prompt = prompt or """Please extract and transcribe all mathematical content from this image.
        Include:
        1. All equations, expressions, and formulas
        2. Any text describing the problem
        3. Diagrams or graphs with their labels
        4. Any constraints or conditions mentioned
        
        Format the output as clean mathematical notation that can be processed by Wolfram Alpha."""
        ...
        try:
            ...
            return {
                ...
                "type": self._classify_content(response)
            }
    ...
    async def enhance_wolfram_result(
        ...
        try:
            explanation = await self.complete(messages, temperature=0.7)
...
This follows the fundamental principle of algebra that you can perform any operation on an equation as long as you do the same thing to both sides."""
    ...
    async def solve_math_problem(
        ...
        """Solve a mathematical problem using GPT-5.
...
Provide a clear answer with brief explanation."""
            ...
            logger.error("Failed to solve math problem with GPT", error=str(e))
            return {
                "success": False,
                "original_query": query,
                "final_answer": None,
    ...
    def _extract_final_answer(self, response: str) -> Optional[str]:
        """Extract the final answer from GPT response.
    ...
    def _extract_steps(self, response: str) -> List[Dict[str, str]]:
        """Extract steps from GPT response.
